...
export class TimeUtils {
  getFreezingTimeNm(freezingMillisecond: number): string {
    const HOUR_NM = FuncUtils.getResourceString($r('app.string.unified_authwidget_hour').id);
    const MINUTE_NM = FuncUtils.getResourceString($r('app.string.unified_authwidget_minutes').id);
    const SECOND_NM = FuncUtils.getResourceString($r('app.string.unified_authwidget_seconds').id);
    const ONE_MINUTE = 60;
    const RATE = 1000;
